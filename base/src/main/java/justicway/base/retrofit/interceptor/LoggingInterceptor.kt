@file:Suppress("PackageDirectoryMismatch", "PropertyName", "unused")

import justicway.base.retrofit.interceptor.Logger
import okhttp3.*
...
class LoggingInterceptor @Inject constructor(private val builder: Builder) : Interceptor {
    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        if (builder.requestFlag) {
            if (request.method == "GET") {
                Logger.printJsonRequest(builder, request, getHeaderString(request.headers))
            } else {
                if (subtypeIsNotFile(rSubtype)) {
                    Logger.printJsonRequest(builder, request, getHeaderString(request.headers))
                } else {
                    Logger.printFileRequest(builder, request, getHeaderString(request.headers))
                }
            }
        }
        ...
        if (builder.responseFlag) {
            val segmentList = (request.tag() as Request?)?.url?.encodedPathSegments ?: emptyList()
            val chainMs = TimeUnit.NANOSECONDS.toMillis(System.nanoTime() - st)
            val header = getHeaderString(response.headers)
    ...
    private fun getHeaderString(headers: Headers): String {
        val headerStringBuilder: StringBuilder = StringBuilder()
        for (key in headers.names()) {
            headerStringBuilder.append(key)
            headerStringBuilder.append(": ")
            headerStringBuilder.append(headers[key])
            headerStringBuilder.append("\n")
        }
    ...
    class Builder {
        ...
        internal fun getTag(isRequest: Boolean) = if (isRequest) {
